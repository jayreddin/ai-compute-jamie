...
class LLM:
    ...
    def get_settings_values(self) -> tuple[str, str, str]:
        ...
        if not base_url.endswith('/v1'): #Ensure the url ends with v1
    ...
    def read_context_txt_file(self) -> str:
        ...
        try:
             ...
             logging.error(f'An unknown error occurred while reading context file: {e}')
             raise
        context += (
    ...
    def get_instructions_for_objective(self, original_user_request: str, step_num: int = 0) -> dict[str, Any]:
        ...
        if self.model_name == "mistral-large":
            ...
            if 'number_of_screenshots' in self.settings_dict and int(self.settings_dict['number_of_screenshots']) > 0 and isinstance(self.model, (GPT4v, GPT4o)):
                for i in range(int(self.settings_dict['number_of_screenshots'])):
                   self.model.get_instructions_for_objective(original_user_request, step_num)
                return {} # the images are added to the queue in the get instructions method.
            else:
                return self.model.get_instructions_for_objective(original_user_request, step_num)
